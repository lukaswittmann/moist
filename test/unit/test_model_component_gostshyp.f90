...
contains
   ...
   subroutine build_fixture_surface(cavity, normals)
   ...
   !> Build the component's coupling, stage its energy phase and answer the
   !> moment request with the model-density moments of the fixture surface
   !>
   !> @param[out]   error     Test failure information
   !> @param[inout] component Component whose moment request the coupling declares
   !> @param[in]    cavity    Synthetic DROP surface
   !> @param[out]   coupling  Coupling staged for the energy phase
   subroutine stage_fixture_moments(error, component, cavity, coupling)
   ...
   subroutine test_gostshyp_energy(error)
      ...
      if (.not. allocated(amplitude%w_overlap) .or. &
          & .not. allocated(amplitude%w_normal_deriv)) then
         call test_failed(error, "GOSTSHYP wrote no host amplitudes")
         return
      end if
      ...
      rebuilt = dot_product(amplitude%w_overlap, gt)
   ...
   !>
   !> Driven with the moment request unanswered, so a missing short circuit
   !> shows up as the error the component raises when it reads its moments
   !>
   !> @param[out]   error     Test failure information
   ...
   subroutine check_inert(error, component, coupling, cavity, mol, label)
      ...
      call check(error, allocated(amplitude%w_overlap), &
         & more="GOSTSHYP dropped its host amplitudes at "//label)
      if (allocated(error)) return
      call check(error, maxval(abs(amplitude%w_overlap)), 0.0_wp, thr=0.0_wp, &
      ...
      call component%get_surface_weights(component_view(coupling), cavity, prefilled, err)
      call check(error, .not. allocated(err), &
   ...
   subroutine test_gostshyp_guards(error)
      ...
      ! Moments never supplied: staged for the energy phase, request unanswered
      call component%new_coupling(cavity, coupling, err)
      if (allocated(err)) then
         call test_failed(error, "GOSTSHYP coupling setup failed: "//err%message)
         return
      ...
      call stage_fixture_moments(error, component, cavity, coupling)
      if (allocated(error)) return
      cavity%ngrid = ngrid_sw + 1
      energy = 7.0_wp
      call component%get_energy(component_view(coupling), cavity, energy, err)
      call check(error, allocated(err), more="live cavity extent mismatch was accepted")
      if (allocated(error)) return
   ...
   !> untouched and only fails at the divisions. Here `pt` alone is scaled
   ...
   subroutine test_gostshyp_nonfinite(error)
      ...
      call check(error, all(ieee_is_finite(amplitude%w_overlap)) &
         & .and. all(ieee_is_finite(amplitude%w_normal_deriv)), &
