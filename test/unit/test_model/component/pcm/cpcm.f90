...
!> cross-checked against the same weights contracted by hand.
   subroutine test_cpcm_surface_weights(error)
      ...
      !> Tessera, nuclear-displacement and Cartesian-component indices
...
!> structure, where a tessera has hundreds of partners spread across both
...
!> matrix assembly and solve included.
   subroutine test_cpcm_molecular_surface_weights(error)
      ...
      !> Tesserae differenced, strided across the exposed surface
      integer, parameter :: n_fd_points = 4
      !> Smallest switching factor a differenced tessera may have.
      !>
      !> An iSwiG surface keeps nearly buried tesserae with f down to ~1e-10, whose
      !> diagonal entry sqrt(2/pi)*xi/f then reaches ~1e10. Their analytic weights
      !> are as valid as any other -- the component treats every tessera alike --
...
!>
!> are active because rebuilding the iSwiG cavity moves its tesserae and changes
...
!>
!> Every iSwiG tessera is placed relative to an atomic centre, so shifting the
...
!> Bit-coincident tesserae must not poison the CPCM matrix.
!>
...
!> zero-separation limit 2 xi_ij/sqrt(pi). This pins that property.
   subroutine test_cpcm_coincident_points(error)
      ...
      !> Three tesserae, the first two bit-identical with distinct widths
      ...
      call check(error, all(ieee_is_finite_matrix(amat)), &
         & "coincident tesserae must not produce non-finite CPCM matrix entries")
      if (allocated(error)) return
      ...
      call check(error, amat(2, 1), expected, thr=thr, &
         & message="the CPCM matrix must stay symmetric at coincident tesserae")
