...
module moist_math_linalg_lsmr

   use mctc_env_accuracy, only: wp, ip => i4
   ...
   abstract interface
      ...
      end subroutine Aprod1_f

      subroutine Aprod2_f(m, n, x, y)
      !! x := x + A'*y
         use mctc_env_accuracy, only: wp, ip => i4
         implicit none
         integer(ip), intent(in)    :: m, n
...
contains
   ...
   subroutine lsmr(m, n, Aprod1, Aprod2, b, damp, &
      ...
      real(wp), intent(in)  :: b(m) !! The rhs vector `b`.
      ...
      real(wp), intent(out) :: normA !! An estimate of the Frobenius norm of Abar.
      ...
      real(wp), intent(out) :: condA !! An estimate of cond(Abar), the condition
                                      !! number of Abar.  A very high value of condA
                                      !! may again indicate an error in Aprod1 or Aprod2.
      real(wp), intent(out) :: normr !! An estimate of the final value of norm(rbar),
                                      !! the function being minimized (see notation
                                      !! above).  This will be small if A*x = b has
                                      !! a solution.
      real(wp), intent(out) :: normAr !! An estimate of the final value of
      ...
      integer(ip) :: i, localOrthoCount, localOrthoLimit, localPointer, localVecs, &
                     pcount, pfreq
      real(wp)    :: alpha, alphabar, alphahat, &
...
!
!  Instead of specifying the `Aprod1`, `Aprod2` functions,
