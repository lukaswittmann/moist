...
module moist_math_linalg_lsqr

   use mctc_env_accuracy, only: wp
   use moist_math_blas, only: nrm2, scal, copy, dot
...
!      if the computation is performed in higher precision.
   ...
   subroutine LSQR(me, m, n, damp, wantse, &
      ...
      if (beta > zero) then
         call scal(u, (one/beta))
         call me%aprod(2, m, n, v, u)
         alpha = nrm2(v)
      end if

      if (alpha > zero) then
         call scal(v, (one/alpha))
         call copy(v, w)
      end if
...
!   tol defined via power.
   ...
   subroutine acheck(me, m, n, nout, eps, &
      ...
      alfa = nrm2(x)
      beta = nrm2(y)
      call scal(x, (one/alfa))
      call scal(y, (one/beta))
...
!***************************************************************************************************
end module moist_math_linalg_lsqr
!***************************************************************************************************
