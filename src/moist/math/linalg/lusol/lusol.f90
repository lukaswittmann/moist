...
! 12 Dec 2011: Had to change ip, iq to p, q to avoid clash with ip, wp.
...
module moist_math_linalg_lusol
   use mctc_env_accuracy, only: wp, ip => i8
   ...
   real(wp), parameter :: zero = 0.0, one = 1.0
...
contains
   ...
   subroutine lu1fac(m, n, nelem, lena, luparm, parmlu, &
      ...
      integer(ip), intent(inout) :: indc(lena), indr(lena), &
                                    ...
                                    ipinv(m), iqinv(n), &
                                    locc(n), locr(m)
      real(wp), intent(inout) :: parmlu(30), a(lena), w(n)
      ...
      real(wp)               :: Agrwth, Akmax, Amax, avgmer, &
...
990   luparm(10) = inform
      ...
      Agrwth = Akmax/(Amax + 1.0e-20_wp)
      Ugrwth = Umax/(Amax + 1.0e-20_wp)
      if (TPP) then
         growth = Ugrwth
      ...
      !------------------------------------------------------------------
      ncp = luparm(26)
      condU = DUmax/max(DUmin, 1.0e-20_wp)
      dincr = lenL + lenU - nelem
      dincr = dincr*100.0_wp/max(delem, one)
      avgmer = mersum
      avgmer = avgmer/dm
      nbump = m - nUtri - nLtri
...
1900  format(/' lu1fac  error...  TSP used but', &
   ...
   subroutine lu1fad(m, n, nelem, lena, luparm, parmlu, &
      ...
      integer(ip), intent(inout) :: luparm(30)
      real(wp), intent(inout) :: parmlu(30), a(lena), Amaxr(m), &
                                 w(n), Ha(lenH)
      integer(ip), intent(inout) :: indc(lena), indr(lena), &
                                    p(m), q(n), &
                                    lenc(n), lenr(m), &
      ...
      real(wp), intent(out)   :: Lmax, Umax, DUmax, DUmin, Akmax
      ...
      real(wp)               :: abest, aijmax, aijtol, amax, &
                                dens1, dens2, diag, &
...
1200  format(' nrowu', i7, '   i,jbest', 2i7, '   nrowd,ncold', 2i6, &
   ...
   subroutine lu1gau(m, melim, ncold, nspare, small, &
      ...
      real(wp), intent(in)    :: al(melim), au(ncold)
...
900   lfirst = lr
   ...
   subroutine lu1mar(m, n, lena, maxmn, &
      ...
      real(wp), intent(in)    :: aijtol, Ltol, a(lena)
      ...
      real(wp)               :: abest, aij, amax, cmax, lbest
      real(wp), parameter :: gamma = 2.0
...
290      if (ibest > 0) then
   ...
   subroutine lu1mRP(m, n, lena, maxmn, &
      ...
      integer(ip), intent(in)    :: m, n, lena, maxmn, maxcol, maxrow
      real(wp), intent(in)    :: Ltol
      integer(ip), intent(in)    :: indc(lena), indr(lena), p(m), q(n), &
                                    lenc(n), lenr(m), iploc(n), iqloc(m), &
                                    locc(n), locr(m)
      real(wp), intent(in)    :: a(lena), Amaxr(m)
      integer(ip), intent(out)   :: ibest, jbest, mbest
      ...
      integer(ip)            :: i, j, kbest, lc, lc1, lc2, len1, &
                                lp, lp1, lp2, lq, lq1, lq2, lr, lr1, lr2, &
                                merit, ncol, nrow, nz, nz1
      real(wp)               :: abest, aij, amax, atoli, atolj
...
290      if (ibest > 0) then
   ...
   subroutine lu1mCP(m, n, lena, aijtol, &
      ...
      real(wp), intent(in)    :: aijtol
      real(wp), intent(in)    :: a(lena), Ha(Hlen)
      integer(ip), intent(out)   :: ibest, jbest, mbest
...
200      if (ibest > 0) then
   ...
   subroutine lu1mxr(mark, k1, k2, m, n, lena, inform, &
      ...
      real(wp), intent(in)    :: a(lena)

      integer(ip), intent(inout) :: markc(n), markr(m)
      real(wp), intent(inout) :: Amaxr(m)
...
500   do k = last + 1, m
   ...
   subroutine lu6sol(mode, m, n, v, w, &
      ...
      integer(ip), intent(inout) :: luparm(30)
      real(wp), intent(inout) :: parmlu(30), v(m), w(n)
   ...
   subroutine lu6L(inform, m, n, v, &
      ...
      integer(ip), intent(inout) :: luparm(30)
      real(wp), intent(inout) :: parmlu(30), v(m)
   ...
   subroutine lu6U(inform, m, n, v, w, &
      ...
      real(wp), intent(in)    :: a(lena)
      real(wp), intent(in)    :: v(m)

      integer(ip), intent(inout) :: luparm(30)
      real(wp), intent(inout) :: parmlu(30)
   ...
   subroutine lu6Ut(inform, m, n, v, w, &
      ...
      real(wp), intent(in)    :: a(lena)
      integer(ip), intent(inout) :: luparm(30)
      real(wp), intent(inout) :: parmlu(30), w(n)
      integer(ip), intent(out)   :: inform
      real(wp), intent(out)   :: v(m)
   ...
   subroutine lu6chk(mode, m, n, nslack, w, lena, luparm, parmlu, &
      ...
      real(wp)            :: aij, diag, DUmax, DUmin, Lmax, Umax, Utol1, Utol2
...
1100  format(' Singular(m', a, 'n)', '  rank', i9, '  n-rank', i8, '  nsing', i9)
   ...
   subroutine lu7add(m, n, jadd, v, lena, luparm, parmlu, &
      ...
      real(wp), intent(inout) :: parmlu(30), a(lena), v(m)
      integer(ip), intent(out)   :: inform, klast
      real(wp), intent(out)   :: vnorm
...
990   return
   ...
   subroutine lu7elm(m, n, jelm, v, lena, luparm, parmlu, &
      ...
      real(wp), intent(inout) :: parmlu(30), a(lena)
...
990   return
   ...
   subroutine lu7for(m, n, kfirst, klast, lena, luparm, parmlu, &
      ...
      integer(ip)           :: ilast, iv, iw, j, jfirst, jlast, jv, &
                               ...
                               minfre, nfree
      real(wp)              :: amult, Ltol, Uspace, small, vj, wj
...
990   return
   ...
   subroutine lu7rnk(m, n, jsing, lena, parmlu, &
      ...
      integer(ip), intent(inout) :: lenL, lenU, lrow, nrank, &
                                    ...
                                    lenr(m), locc(n), locr(m)
      real(wp), intent(inout) :: parmlu(30)  ! not used
      real(wp), intent(inout) :: a(lena)
      integer(ip), intent(out)   :: inform
      real(wp), intent(out)   :: diag
...
1800  format(/' lu8rpc  error...  jrep  is out of range.', &
   ...
   function jdamax(n, x, incx) result(iAmax)
      ...
      real(wp)            :: dmax, xi
      real(wp), parameter :: realmax = huge(1.0_wp)
...
end module moist_math_linalg_lusol
