...
module nlesolver_module
   ...
   use moist_math_lapack_gels, only: dgels
...
!    if m<n uses LQ factorization)
   ...
   subroutine linear_solver(m, n, a, b, x, info)
      ...
      integer(lapack_ik), dimension(:), allocatable :: ipiv     !! pivot indices array
      ...
      integer(lapack_ik)                  :: lwork  !! size of `work`
      integer(lapack_ik)                  :: info_lapack, n_lapack, m_lapack

      allocate (amat(m, n))
      allocate (bmat(max(1, m, n), 1))

      n_lapack = int(n, lapack_ik)
      m_lapack = int(m, lapack_ik)
      info = 0
      ...
      if (n == m) then  !normal inverse
         ...
         amat = a
         bmat(1:n, 1) = b
         call dgesv(n_lapack, int(1, lapack_ik), amat, n_lapack, ipiv, bmat, n_lapack, info_lapack)
         x = bmat(1:n, 1)
      ...
      if (info_lapack > int(huge(info), lapack_ik)) then
         info = huge(info)
      else if (info_lapack < -int(huge(info), lapack_ik)) then
         info = -huge(info)
      else
...
!  Usually this is overkill and not necessary, but is here as an option for testing.
   ...
   subroutine exact_linesearch(me, xold, p, x, f, fvec, fjac, fjac_sparse)
      ...
      ! Resolve the (possibly bound-adjusted) search direction BEFORE the line
      ! search. fmin evaluates func_for_fmin, which steps along search_direction
      ! and consults `modified`, so both must be set first. Previously this call
      ! came after fmin, so the minimization ran over uninitialised arrays --
