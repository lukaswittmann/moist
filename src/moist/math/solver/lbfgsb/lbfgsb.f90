...
module lbfgsb_module
   ...
   use moist_math_blas_legacy, only: daxpy, dcopy, ddot, dscal
...
!    Brett M. Averick, Richard G. Carter, and Jorge J. More'.
   ...
   subroutine dcsrch(f, g, Stp, Ftol, Gtol, Xtol, Stpmin, Stpmax, Task, Isave, Dsave)
      ...
      if (Stp == Stpmax .and. f <= ftest .and. g <= gtest) then
         Task = "WARNING: STP = STPMAX"
      end if
      if (Stp == Stpmin .and. (f > ftest .or. g >= gtest)) then
