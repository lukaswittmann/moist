...
contains
   ...
   pure subroutine wrap_sgemv(amat, xvec, yvec, alpha, beta, trans)
      ...
      real(sp) :: a, b
      character(len=1) :: tra
      integer(blas_ik) :: incx, incy, m, n, lda
   ...
   pure subroutine wrap_ssymv(amat, xvec, yvec, uplo, alpha, beta)
      ...
      character(len=1) :: ula
      real(sp) :: a, b
      integer(blas_ik) :: incx, incy, n, lda
      if (present(alpha)) then
         a = alpha
      else
         a = 1.0_sp
...
!> Wrapper for rank-1 update: A := alpha*x*y^T + A (single precision)
   pure subroutine wrap_sger(xvec, yvec, amat, alpha)
      real(sp), intent(in) :: xvec(:)
      real(sp), intent(in) :: yvec(:)
      real(sp), intent(inout) :: amat(:, :)
      real(sp), intent(in), optional :: alpha
      real(sp) :: a
      integer(blas_ik) :: m, n, incx, incy, lda
   ...
   end subroutine wrap_sger

!> Wrapper for rank-1 update: A := alpha*x*y^T + A (double precision)
   pure subroutine wrap_dger(xvec, yvec, amat, alpha)
      real(dp), intent(in) :: xvec(:)
      ...
      incy = 1
      call blas_ger(m, n, a, xvec, incx, yvec, incy, amat, lda)
   end subroutine wrap_dger

end module moist_math_blas_level2
